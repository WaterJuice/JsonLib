...
These are complete, self-contained programs. Only `JsonLib.h` needs to be
included. Link against the `JsonLib` static library.

## Round trip: JSON to struct and back

Parse a JSON document into a struct, modify it, and write it back out as
indented JSON.
...
static size_t const PersonMapCount =
    sizeof(PersonMap) / sizeof(PersonMap[0]);
...
int main( void )
{
    ...
    JL_STATUS status = JlJsonToStruct(
        json, PersonMap, PersonMapCount, &person, &errorPos );
    if( JL_STATUS_SUCCESS != status )
    {
        printf( "Parse failed at %zu (status %d)\n",
                errorPos, (int)status );
        return 1;
    }
    ...
    // Modify a field and write the struct back as indented JSON.
    person.Age += 1;

    char* output = NULL;
    status = JlStructToJson(
        &person, PersonMap, PersonMapCount, true, &output );
    if( JL_STATUS_SUCCESS == status )
    {
        printf( "%s\n", output );
    ...
    // Free everything the parse allocated in the struct (Name).
...
A struct can contain another struct, allocated arrays of strings or numbers, and
more. The input below uses JSON5 (bare keywords and single quotes), which is
accepted by default.
...
static size_t const AddressMapCount =
    sizeof(AddressMap) / sizeof(AddressMap[0]);

static JlMarshallElement ProfileMap[] =
{
    JlMarshallString( Profile, Name, "name" ),
    JlMarshallStruct( Profile, Home, "home",
                      AddressMap, AddressMapCount ),
    JlMarshallStringArray( Profile, Tags, TagsCount, "tags" ),
    JlMarshallUnsignedArray( Profile, Scores, ScoresCount, "scores" ),
};
static size_t const ProfileMapCount =
    sizeof(ProfileMap) / sizeof(ProfileMap[0]);
...
int main( void )
{
    ...
    size_t errorPos = 0;
    JL_STATUS status = JlJsonToStruct(
        json, ProfileMap, ProfileMapCount, &profile, &errorPos );
    if( JL_STATUS_SUCCESS != status )
    {
        printf( "Parse failed at %zu\n", errorPos );
        return 1;
    }

    printf( "%s lives in %s. %zu tags, %zu scores.\n",
            profile.Name, profile.Home.City,
            profile.TagsCount, profile.ScoresCount );
...
`JlStructToJsonEx` takes a set of flags. Combine them to choose indentation,
JSON5 features, and more. For example, to produce compact JSON5 with bare
keywords and single quotes:
